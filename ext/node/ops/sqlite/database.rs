...
impl DatabaseSync {
  ...
  fn apply_changeset<'a>(
    ...
    unsafe {
      let r = libsql_ffi::sqlite3changeset_apply(
        raw_handle,
      ...
      if r == libsql_ffi::SQLITE_OK {
        return Ok(true);
      } else if r == libsql_ffi::SQLITE_ABORT {
        return Ok(false);
      }
  ...
  fn create_session(
    ...
    let r = unsafe {
      libsql_ffi::sqlite3session_create(
        raw_handle,
    ...
    let r =
      ...
      unsafe { libsql_ffi::sqlite3session_attach(raw_session, z_table) };
